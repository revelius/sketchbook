        ...
        function create_goodInfo()
        {
        	...
        	console.log('${listCount.count}');
	        	<c:if test="${!listCount.last}">
	        		good_info += "seq=${listCount.count}" + chr31 + "ordr_numb=${ordNo}_${listCount.count}" + chr31 + "good_name=${goods.ubiGdsNm}" + chr31 + "good_cntx=1" + chr31 + "good_amtx=${goods.slPrc - goods.cpnPayAmt}" + chr30;
	        	</c:if>
	        	<c:if test="${listCount.last}">
	        		good_info += "seq=${listCount.count}" + chr31 + "ordr_numb=${ordNo}_${listCount.count}" + chr31 + "good_name=${goods.ubiGdsNm}" + chr31 + "good_cntx=1" + chr31 + "good_amtx=${goods.slPrc - goods.cpnPayAmt}";
	        	</c:if>
        	</c:forEach>
...
<form name="order_info" method="post" action="orderPayWeb" >
...
<%
                    ...
                    /* = -------------------------------------------------------------------------- = */
%>
					<input type="text" name="rsvPnt" value="${order}"/>


					<c:if test="${order.orderPay[0].payWay == 'OR00801'}">
					신용카드 : <input type="text" name="pay_method"  value="100000000000"/><br/>
					</c:if>
					<c:if test="${order.orderPay[0].payWay == 'OR00802'}">
					계좌이체 : <input type="text" name="pay_method"  value="010000000000"/><br/>
					</c:if>
					<c:if test="${order.orderPay[0].payWay == 'OR00803'}">
					가상계좌 : <input type="text" name="pay_method"  value="001000000000"/><br/>
					</c:if>
					주문번호 : <input type="text" name="ordr_idxx" value="${order.ordNo }" maxlength="40"/><br/>
					상품명 : <input type="text" name="good_name" value="[${order.orderGoods[0].brnd }]${order.orderGoods[0].ubiGdsNm } 포함 ${fn:length(order.orderGoods)} 개"/><br/>
					결제금액 : <input type="text" name="good_mny" value="${totalSlAmt+deliveryCost-order.orderPay[1].payAmt }" maxlength="9"/><br/>
					주문자명 : <input type="text" name="buyr_name" value="${order.ordrNm }"/><br/>
					E-mail : <input type="text" name="buyr__mail" value="${order.ordrEmail }"/><br/>
					전화번호 : <input type="text" name="buyr_tel1" value="${order.ordrTel }"/><br/>
					휴대폰번호 : <input type="text" name="buyr_tel2" value="${order.ordrCp }"/><br/>
<%
     /* ============================================================================== */
     /* =   1-2. 에스크로 정보                                                       = */
     /* = -------------------------------------------------------------------------- = */
...
%>
					...
					수취인명 : <input type="text" name="rcvr_name" value="${order.rcpnNm }"/><br/>
					전화번호 : <input type="text" name="rcvr_tel1" value="${order.rcpnTelNo }"/><br/>
					휴대폰번호 : <input type="text" name="rcvr_tel2" value="${order.rcpnCpNo }"/><br/>
					수취인 E-mail : <input type="text" name="rcvr_mail" value="${order.ordrEmail }"/><br/>
					수취인 우편번호 : <input type="text" name="rcvr_zipx" value="${order.rcpnZipCd }"/><br/>
					수취인 주소 : <input type="text" name="rcvr_add1" value="${order.rcpnBsAddr }"/><br/>
					수취인 상세주소 : <input type="text" name="rcvr_add2" value="${order.rcpnDtlAddr }"/><br/>
<%
     /* = -------------------------------------------------------------------------- = */
     /* =   1-2. 에스크로 정보  END													 = */
     /* ============================================================================== */
...
<%
    ...
    /* ============================================================================== */
%>
	<input type="hidden" name="vcnt_expire_term" value="1"/>
	<input type='hidden' name='skin_indx'      value='5'>
</form>
</body>

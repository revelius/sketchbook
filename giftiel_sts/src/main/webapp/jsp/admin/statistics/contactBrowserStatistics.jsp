<%--
 * =============================================================================
 * 파일명 : /statistics/contactBrowserStatistics.jsp
 * 작성자 : 이현우
 * 작성일자 : 2015-07-10
 *
 * 파일 내용 : 통계 메인
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<form:form id="schForm" name="schForm" method="POST" modelAttribute="searchForm" onsubmit="">
				<form:hidden path="dateType" name="dateType" id="dateType" value="Browser"/>
				...
				<div class="section a2">
					<div class="tableTitle">
						...
						<div class="right">
							...
							<form:select path="searchCount" id="searchCount" onchange="javascript:doNumList();" cssClass="ml_5">
					...
					<!-- 접속경로, 브라우저, OS 버튼클릭 시 폼 display: none으로 숨겨두었음-->
						...
						<table>
							...
							<tfoot>
								<tr class="txtBold">
									...
									<td class="tdBg">${totalVisit }</td>
									<td class="tdBg"></td>
								</tr>
							</tfoot>
							<tbody>
								<c:choose>
									<c:when test="${not empty selectVisitEtc}">
										<c:forEach items="${selectVisitEtc}" var="visitBrowser" varStatus="listCount">
											<tr>
												<td>${visitBrowser.rnum }</td>
												<td style="word-break:break-all;">${visitBrowser.connBrws }</td>
												<td>${visitBrowser.visitCnt }</td>
												<td><span class="graphArea"><span class="barRed" style="width: ${visitBrowser.visitCnt/100 }%; max-width:100%;">${visitBrowser.visitCnt }%</span></span></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>

<%--
 * =============================================================================
 * 파일명 : /statistics/contactDomainStatistics.jsp
 * 작성자 : 이현우
 * 작성일자 : 2015-07-10
 *
 * 파일 내용 : 통계 메인
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<form:form id="schForm" name="schForm" method="POST" modelAttribute="searchForm" onsubmit="">
				<form:hidden path="dateType" name="dateType" id="dateType" value="Domain"/>
							...
							<tbody>
								<c:choose>
									<c:when test="${not empty selectVisitEtc}">
										<c:forEach items="${selectVisitEtc}" var="visitDomain" varStatus="listCount">
											<tr>
												<td>${visitDomain.rnum }</td>
												<td style="word-break:break-all;">${visitDomain.connDomain }</td>
												<td>${visitDomain.visitCnt }</td>
												<td><span class="graphArea"><span class="barRed" style="width: ${visitDomain.visitCnt }%; max-width:100%">${visitDomain.visitCnt }%</span></span></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>

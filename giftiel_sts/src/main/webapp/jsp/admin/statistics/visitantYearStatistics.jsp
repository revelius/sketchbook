<%--
 * =============================================================================
 * 파일명 : /statistics/visitantYearStatistics.jsp
 * 작성자 : 이현우
 * 작성일자 : 2015-07-10
 *
 * 파일 내용 : 통계 메인
...
<head>
	...
	<script type="text/javascript">
	    $(function() {
	    	var sD = $("#searchStrDtm").val();
	    	var sY = sD.split(".")[0];
	    	var sM = sD.split(".")[1];
	    	
	    	$("#sYear > option[value=" + sY + "]").attr("selected", "true");
	    	$("#sMonth > option[value=" + sM + "]").attr("selected", "true");
	    	
	    	var eD = $("#searchEndDtm").val();
	    	var eY = eD.split(".")[0];
	    	var eM = eD.split(".")[1];
	    	
	    	$("#eYear > option[value=" + eY + "]").attr("selected", "true");
	    	$("#eMonth > option[value=" + eM + "]").attr("selected", "true");
		    ...
		    goSubmit = function(){
		    	...
		    	// 날짜 가공
		    	// 검색 월을 두자리로만들어 뒤에서부터 잘라 문자열로 합침
				var sTwoMonth = "0" + $("#sMonth").val();
				var eTwoMonth = "0" + $("#eMonth").val();
				var searchStrDtm = $("#sYear").val() + "." + sTwoMonth.substring(sTwoMonth.length-2,3) + ".01";
				var searchEndDtm = $("#eYear").val() + "." + eTwoMonth.substring(eTwoMonth.length-2,3) + ".01";
				
				$("#searchStrDtm").val(searchStrDtm);
				$("#searchEndDtm").val(searchEndDtm);
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<form:form id="schForm" name="schForm" method="post" modelAttribute="searchForm" onsubmit="">
				<form:hidden path="searchStrDtm" id="searchStrDtm" name="searchStrDtm" />
				<form:hidden path="searchEndDtm" id="searchEndDtm" name="searchEndDtm" />
								...
								<tr>
									...
									<td>
										...
										<select id="sYear" name="sYear">
											<c:forEach begin="2002" end="2015" step="1" var="year">
												<option value="${year}">${year}</option>
											</c:forEach>
										</select> 년
										<select id="sMonth" name="sMonth">
											<c:forEach begin="1" end="12" step="1" var="month">
												<option value="<c:if test="${month < 10}">0</c:if>${month }"><c:if test="${month < 10}">0</c:if>${month}</option>
											</c:forEach>
										</select> 월~
									...
									<c:when test="${not empty statisticsConnList}">
										<c:forEach var="list" items="${statisticsConnList}" varStatus="stat">
											...
											<tr>
												...
												<span class="<c:if test="${brand.connBrnd != '합계'}">${cssClass }</c:if>" style="width: ${brand.brandTot div 1000}%; max-width:100%;"></span></span></td>
											</tr>
											</c:forEach>
										</c:forEach>
									</c:when>

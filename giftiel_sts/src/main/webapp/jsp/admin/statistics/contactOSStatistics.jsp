<%--
 * =============================================================================
 * 파일명 : /statistics/contactOSStatistics.jsp
 * 작성자 : 이현우
 * 작성일자 : 2015-07-10
 *
 * 파일 내용 : 통계 메인
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<form:form id="schForm" name="schForm" method="POST" modelAttribute="searchForm" onsubmit="">
				<form:hidden path="dateType" name="dateType" id="dateType" value="OS"/>
							...
							<tbody>
								<c:choose>
									<c:when test="${not empty selectVisitEtc}">
										<c:forEach items="${selectVisitEtc}" var="visitOS" varStatus="listCount">
											<tr>
												<td>${visitOS.rnum }</td>
												<td style="word-break:break-all;">${visitOS.connOs }</td>
												<td>${visitOS.visitCnt }</td>
												<td><span class="graphArea"><span class="barRed" style="width: ${visitOS.visitCnt/100 }%; max-width:100%;">${visitOS.visitCnt }%</span></span></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>

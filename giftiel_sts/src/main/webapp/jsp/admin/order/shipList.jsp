<%--
 * =============================================================================
 * 파일명 : /order/shipList.jsp
 * 작성자 : 박필종
 * 작성일자 : 2015-05-07
 *
 * 파일 내용 : 주문매출관리 > 배송관리 리스트
 * =============================================================================
--%>
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<jsp:include page="/jsp/admin/include/leftMenu.jsp">
					...
					<jsp:param name="onChk" value="shipList"/>
				</jsp:include>
				<!--[e] lnb -->
				...
				<form:form id="schForm" name="schForm" method="post" modelAttribute="searchForm" onsubmit="return check(form)">
					...
					<form:hidden path="ordGdsSeqs"/><!-- 주문상품일련번호 다건 -->
					...
					<div class="contents">
						...
						<div class="section a1">
							<h3><span class="icon1"></span>배송관리</h3>						
							<div class="tableType2">
								<p class="boxP">배송 관리할 수 있는 곳입니다.</p>
									<!-- [s] 검색조건 -->
								<table>
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<div class="wrap">
													<c:forEach items="${perOrdStatList}" var="perOrdStat" varStatus="perOrdStatCount" end="5"> 
											...
											<c:choose>
												<c:when test="${not empty orderList}">
													<c:forEach items="${orderList}" var="order" varStatus="listCount">
														<tr>
															<td>
																<c:if test="${order.perOrdStat=='OR00501'}"><!-- 결제대기 -->
																	
																</c:if>
																<c:if test="${order.perOrdStat=='OR00502'}"><!-- 결제완료 -->
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00503', '${order.orderPay[0].payWay }'); return false;">배송준비</a>
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00506', '${order.orderPay[0].payWay }'); return false;">출고지연</a>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00503'}"><!-- 배송준비 -->
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00506', '${order.orderPay[0].payWay }'); return false;">출고지연</a>
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', 'STE1', 'OR00504', '${order.orderPay[0].payWay }'); return false;">배송중</a>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00504'}"><!-- 배송중 -->
																	<a href="#" class="btnType1">배송추적</a>
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00505', '${order.orderPay[0].payWay }'); return false;">배송완료</a>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00505'}"><!-- 배송완료 -->
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00508', '${order.orderPay[0].payWay }'); return false;">교환신청</a>
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00512', '${order.orderPay[0].payWay }'); return false;">반품신청</a>
																	<a href="#" class="btnType1">배송추적</a>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00506'}"><!-- 줄고지연 -->
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00503', '${order.orderPay[0].payWay }'); return false;">배송준비</a>
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', 'STE1', 'OR00504', '${order.orderPay[0].payWay }'); return false;">배송중</a>
																</c:if>
															</td>
															...
															<td>${order.siteNm}</td>
															<td onclick="javascript:gotoOrderPopup('${order.ordNo }')" style="cursor:pointer">${order.ordNo}
															</td>
															...
															<td class="tl">
																<div class="float_con">
																	...
																	<div class="leftbox">
																		...
																		<br>
																		<input type="text" value="${order.invNo}"> 
																	</div>
															...
															<td>
																<c:if test="${order.orderPay[0].payWay == 'OR00801'}"><!-- 신용카드 -->
																	<c:set var="modType" value="STSC"/>
																</c:if>
																<c:if test="${order.orderPay[0].payWay == 'OR00802'}"><!-- 실시간계좌 -->
																	<c:set var="modType" value="STE2"/>
																</c:if>
																<c:if test="${order.orderPay[0].payWay == 'OR00803'}"><!-- 가상계좌 -->
																	<c:set var="modType" value="STE2"/>
																	<c:if test="${order.ordStat == 'OR00202'}"><!-- 가상계좌 입금전취소 -->
																		<c:set var="modType" value="STE5"/>
																	</c:if>
																</c:if>															
															
																<c:if test="${order.perOrdStat=='OR00501' || order.perOrdStat=='OR00502' || order.perOrdStat=='OR00503' || order.perOrdStat=='OR00506'}">
																	<a href="#" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '', '${modType }', 'OR00507', '${order.orderPay[0].payWay }'); return false;">주문취소</a>
																</c:if>															
															</td> 
														</tr>
													</c:forEach>	

<%--
 * =============================================================================
 * 파일명 : /order/addBuyList.jsp
 * 작성자 : 박필종
 * 작성일자 : 2015-05-07
 *
 * 파일 내용 : 주문매출관리 > 추가결제관리 리스트
 * =============================================================================
--%>
...
<head>
	...
	<script type="text/javascript">
		$(function() {
		    ...
		    //결제진행상태 전체체크
		    $("#allCheckPayStat").on("click" ,function(){
				if($("#allCheckPayStat").is(":checked")) {
					$("input[name=payStats]").prop("checked",true);
				} else {
					$("input[name=payStats]").prop("checked",false);
				}
			});
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<jsp:include page="/jsp/admin/include/leftMenu.jsp">
					...
					<jsp:param name="onChk" value="addBuyList"/>
				</jsp:include>
				<!--[e] lnb -->
				...
				<form:form id="schForm" name="schForm" method="post" modelAttribute="searchForm" onsubmit="return check(form)">
					...
					<div class="contents">
						...
						<div class="section a1">
							<h3><span class="icon1"></span>추가결제관리</h3>						
							<div class="tableType2">
								<p class="boxP">추가결제 내역을 관리 할 수 있는 곳입니다.</p>
								<!-- [s] 검색조건 -->
								<table>
									...
									<tbody>
										<tr>
											<th><input type="checkbox" id="allCheckPayStat" name="allCheckPayStat" /></th>
											<th>진행상태</th>
											<td>
												<div class="wrap">
													<c:forEach items="${payStatList}" var="payStat" varStatus="payStatCount">
														<form:checkbox path="payStats" value="${payStat.cdNo}"/><label for="perOrdStat${payStatCount.count}">${payStat.cdNm}</label>
													</c:forEach>
												</div>
											</td>
										</tr>
										...
										<tr>
											<th><input type="checkbox" id="" name="" disabled="disabled"/></th>
											<th>결제금액</th>
											<td><form:input path="addPayStrPrc" id="addPayStrPrc"/> &nbsp;원 ~ <form:input path="addPayEndPrc" id="addPayEndPrc"/> &nbsp;원 </td>
										</tr>
										<tr>
											<th><input type="checkbox" id="" name="" disabled="disabled"/></th>
											<th>직접검색</th>
											<td>
												<form:select path="searchGb">
													...
													<form:option value="ADD_PAY.ORD_NO">주문번호</form:option> 
													<form:option value="ADD_PAY.ADD_PAY_RSN">결제사유</form:option>
												</form:select>
							...
							<div class="section a2">
								...
								<div class="tableType1" id="orderTable">
									<table>
										...
										<thead>
											<tr id="sortTr">
												<th>진행상태<a id="PAY_STAT_NM" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												<th>결제사유<a id="ADD_PAY_RSN" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												<th>주문번호<a id="ORD_NO" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												<th>금액<a id="PAY_AMT" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												...
												<th>요청일시<a id="REG_DTS" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												<th>결제일시<a id="PAY_DTS" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
											</tr>
										</thead>
										<tbody>
											<c:choose>
												<c:when test="${not empty addPayList}">
													<c:forEach items="${addPayList}" var="addPay" varStatus="listCount">
														<tr>
															<td>${addPay.payStatNm}</td>
															<td>
																${fn:substring(addPay.addPayRsn,0,13)} 
																<c:if test="${fn:length(addPay.addPayRsn) > 13}">····</c:if>
															</td>
															<td onclick="javascript:gotoOrderPopup('${addPay.ordNo }')" style="cursor:pointer">${addPay.ordNo}</td>
															<td><fmt:formatNumber type="currency" value="${addPay.addPayAmt}" pattern="###,###" />원</td>
															<td>${addPay.email}</td>
															<td>${addPay.nm}</td>
															<td>
																<c:choose>
																	<c:when test="${empty addPay.regDts}">-</c:when>
																	<c:otherwise><fmt:formatDate value="${addPay.regDts}" pattern="yyyy-MM-dd HH:mm:SS" /></c:otherwise>
																</c:choose>
															</td>
															<td>
																<c:choose>
																	<c:when test="${empty addPay.payDts}">-</c:when>
																	<c:otherwise><fmt:formatDate value="${addPay.payDts}" pattern="yyyy-MM-dd HH:mm:SS" /></c:otherwise>
																</c:choose>
															</td>
														</tr>
													</c:forEach>	

<%--
 * =============================================================================
 * 파일명 : /order/takeBackList.jsp
 * 작성자 : 박필종
 * 작성일자 : 2015-05-07
 *
 * 파일 내용 : 주문매출관리 > 반품관리 리스트
 * =============================================================================
--%>
...
<head>
	...
	<script type="text/javascript">
		...
		//주문상품 배송상태변경
		//ordNo, ordGdsSeq, modType, perOrdStat, payWay, dlvCls
		updateOrderProcess = function() {
		    ...
		    fm = add_input(frm,'ordNo',arguments[0]);
		    fm = add_input(frm,'ordGdsSeq',arguments[1]);
		    fm = add_input(frm,'modType',arguments[2]);
		    fm = add_input(frm,'perOrdStat',arguments[3]);
		    fm = add_input(frm,'payWay',arguments[4]);
		    if(arguments[5] != null){
		    	fm = add_input(frm,'dlvCls',arguments[5]);
		    }
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<jsp:include page="/jsp/admin/include/leftMenu.jsp">
					...
					<jsp:param name="onChk" value="takeBackList"/>
				</jsp:include>
				<!--[e] lnb -->
				...
				<form:form id="schForm" name="schForm" method="post" modelAttribute="searchForm" onsubmit="return check(form)">
					...
					<div class="contents">
						...
						<div class="section a1">
							<h3><span class="icon1"></span>반품관리</h3>						
							<div class="tableType2">
								<p class="boxP">고객이 반품 신청한 내역을 보여주는 곳입니다.</p>
								<!-- [s] 검색조건 -->
								<table>
									<caption>반품관리</caption>
									<colgroup>
										<col width="56px" />
										<col width="132px" />
										<col width="*" />
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<div class="wrap">
													<c:forEach items="${perOrdStatList}" var="perOrdStat" varStatus="perOrdStatCount" begin="11"> 
											...
											<c:choose>
												<c:when test="${not empty orderList}">
													<c:forEach items="${orderList}" var="order" varStatus="listCount">
														<tr>
															<td>
															<c:set var="modeType" value=""/>
															<c:if test="${order.orderPay[0].payWay == 'OR00801'}"><!-- 신용카드 -->
																<c:set var="modeType" value="STPC"/>
															</c:if>
															<c:if test="${order.orderPay[0].payWay == 'OR00802'}"><!-- 실시간계좌 -->
																<c:set var="modeType" value="STE9_AP"/>
															</c:if>
															<c:if test="${order.orderPay[0].payWay == 'OR00803'}"><!-- 가상계좌 -->
																<c:set var="modeType" value="STE9_VP"/>
															</c:if>
																<c:if test="${order.perOrdStat=='OR00512'}"><!-- 반품신청 -->
																	<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '${modeType }', 'OR00513', '${order.orderPay[0].payWay }'); return false;">반품진행</a>
																	<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '${modeType }', 'OR00514', '${order.orderPay[0].payWay }'); return false;">반품처리</a>
																	<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '${modeType }', 'OR00515', '${order.orderPay[0].payWay }'); return false;">반품불가</a>
																	<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '', 'OR00505', '${order.orderPay[0].payWay }'); return false;">배송완료</a>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00513'}"><!-- 반품진행 -->
																	<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '${modeType }', 'OR00514', '${order.orderPay[0].payWay }'); return false;">반품처리</a>
																	<c:if test="${order.orderDelivery[0].orderDlvCsts[0].actlDlvCst != ''}">
																		<a href="#none" class="btnType1" onclick="updateOrderProcess('${order.ordNo}', '${order.ordGdsSeq}', '${modeType }', 'OR00514', '${order.orderPay[0].payWay }', 'true'); return false;">반품처리(배송비포함)</a>
																	</c:if>
																</c:if>
																<c:if test="${order.perOrdStat=='OR00514'}"><!-- 반품완료 -->
																	-
																</c:if>
																<c:if test="${order.perOrdStat=='OR00515'}"><!-- 반품완료 -->
																	-
																</c:if>
															</td>

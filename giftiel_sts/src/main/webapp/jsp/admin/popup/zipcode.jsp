...
$(function() { 
	...
	$('.addr_item').on("click",function(){
		var zip = $(this).attr("data-value").split("-");
		var address= $(this).text();
		console.log(zip ,  address);
		
// 		opener.document.frm.post_no.value = zip;
// 		opener.document.frm.address.value = address;
		$(opener.document).find("Form").find("#zipCd1").val(zip[0]);
		$(opener.document).find("Form").find("#zipCd2").val(zip[1]);
		$(opener.document).find("Form").find("#bsAddr").val(address);
		self.close();
	});	
...
function getSigungu(){
	...
	$.ajax({
        url : "<c:url value='/front/popup/zipCode/sigunguList'/>",
        type: "post",
        data : formdata,
        async: false,

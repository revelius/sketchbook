...
<body style="font-size:12px;margin:0;padding:0;background-color:#ffffff">
	<table cellpadding="0" cellspacing="0" style="font-size:12px;width:720px;margin:0 auto;padding:0;border:0;font-family:Arial;background-color:#fff;text-align: center;">
		...
		<tr>
			<td style="font-size:12px;text-align: left;font-weight: normal;padding:10px 0 0 10px;">주문한 내역을 취소 처리 하였습니다.</td>
		</tr>
		<tr>
			<td style="font-size:12px;text-align: left;font-weight: normal;padding:30px 0 0 10px">번호 : ${order.ordNo}</td>
		</tr>
		<tr>
			<td style="font-size:12px;text-align: left;font-weight: normal;padding:10px 0 0 10px">주문일자 : <fmt:formatDate value="${order.ordDts}" pattern="yyyy-MM-dd HH:mm"/></td>
		</tr>
	</table>
...
<%-- 			<c:if test="${good.ordGdsSeq eq ordGdsSeq}"> --%>
			<c:set var="cnt" value="${cnt+1}"/>
				<tr>
					<td style="width:50px;font-size:12px;text-align: center;font-weight: normal;border-right:1px solid #e1e1e1;border-top:1px solid #e1e1e1;padding:20px 0 20px 0;color:#000000;">${cnt}</td>
...
<%-- 			</c:if> --%>
	...
	<table cellpadding="0" cellspacing="0" style="font-size:12px;width:720px;margin:0 auto 0 auto;padding:0 0 0 0;border:1px solid #e1e1e1;font-family:Arial;background-color:#fff;text-align: center;">
		<tr>
			<td style="width:30%;font-size:12px;text-align: center;font-weight: normal;padding:10px 0 10px 0;background-color:#f0f0f0;color:#818181;">총 결제금액</td>
			<td style="width:40%;font-size:12px;text-align: center;font-weight: normal;padding:10px 0 10px 0;background-color:#f0f0f0;color:#818181;">환불예정금액</td>
			<td style="width:30%;font-size:12px;text-align: center;font-weight: normal;padding:10px 0 10px 0;background-color:#f0f0f0;color:#818181;">사용 포인트 반환</td>
		</tr>
		<tr>
			<td style="width:30%;font-size:12px;text-align: center;font-weight: normal;border-right:1px solid #e1e1e1;padding:20px 0 20px 0;color:#000000;"><fmt:formatNumber type="currency" value="${order.totalSlAmt}" pattern="###,###" /></td>
			<td style="width:40%;font-size:12px;text-align: center;font-weight: normal;border-right:1px solid #e1e1e1;padding:20px 0 20px 0;color:#000000;">
				<c:forEach var="good" items="${order.orderGoods}" varStatus="goodIndex">
					<c:if test="${good.ordGdsSeq eq ordGdsSeq}">
						<span class="ico_price"><fmt:formatNumber type="currency" value="${good.actlPayPrc}" pattern="###,###" /></span>
					</c:if>
				</c:forEach>
			</td>
			<td style="width:30%;font-size:12px;text-align: center;font-weight: normal;border-right:1px solid #e1e1e1;padding:20px 0 20px 0;color:#000000;">
				<c:forEach var="good" items="${order.orderGoods}" varStatus="goodIndex">
					<c:if test="${good.ordGdsSeq eq ordGdsSeq}">
	...
	<table cellpadding="0" cellspacing="0" style="font-size:12px;width:720px;margin:20px auto 0 auto;padding:0 0 15px 0;border:1px solid #e1e1e1;font-family:Arial;background-color:#fff;text-align: center;">
		...
		<tr>
			<td style="vertical-align:top;width:125px;font-size:12px;text-align: left;font-weight: normal;padding:20px 0 15px 30px;color:#000000;">
				<img style="vertical-align:top" src="/html/worksout/images/common/bu_email_dot.gif" alt="" width="2" height="11" border="0" align="absmiddle" />
				결제수단
			</td>
			<td style="width:595px;font-size:12px;text-align: left;font-weight: normal;padding:20px 0 15px 0;color:#000000;">

<%--
 * =============================================================================
 * 파일명 : /manage/obeyAwarenessDetail.jsp
 * 작성자 : 김진성
 * 작성일자 : 2015-08-04
 *
 * 파일 내용 : obeyAwareness 상세
 * =============================================================================
--%>
...
<head>
    ...
    $(function(){
    	...
    	var awnesFormJson = ${awnesFormJson};
    	...
    	$(document).on("click","#btn_update",function(e){ 
	    	...
	    	$.ajax({
   		        url : "<c:url value='/admin/manage/obeyAwarenessUpdateAjax'/>",
   		        type: "post",
    	...
    	$(document).on("click","#btn_del",function(e){ 
	    	...
	    	$.ajax({
   		        url : "<c:url value='/admin/manage/obeyAwarenessDelAjax'/>",
   		        type: "post",
   		        data : $("form#awnesForm").serialize(),
   		        async: false,
   		        success : function(res){
    	...
    	var htmlRow = "<tbody data-cont>" + $("[data-cont]").first().html().split("awnesCont").join("awnesCont[1000]") + "</tbody>" ; 
		$("[data-cont]").first().remove();
    	for(var i=0;i< awnesFormJson.awnesCont.length;i++){
    		$("#sortTable").append(htmlRow);
	    	$("#sortTable").find("tbody").last().find("img").attr("src","");
	    	$("#sortTable").find("tbody").last().find("input[id^='imgUrl']").val("");
	    	$("#sortTable").find("tbody").last().find("input[name$='contUrl']").val("");
	    	$("#sortTable").find("tbody").last().find(":checked",true).attr("checked",false);
	    	initCont();
    		$("#sortTable").find("tbody").last().find("[value="+awnesFormJson.awnesCont[i].contCls+"]").trigger("click");
    		$("#sortTable").find("tbody").last().find("div[id^='cls']:visible",true).find("input[name$='imgUrl']").val(awnesFormJson.awnesCont[i].imgUrl);
    		$("#sortTable").find("tbody").last().find("div[id^='cls']:visible",true).find("input[name$='contUrl']").val(awnesFormJson.awnesCont[i].contUrl);
    		$("#sortTable").find("tbody").last().find("div[id^='cls']:visible",true).find("img").attr("src","${uploadImage }"+awnesFormJson.awnesCont[i].imgUrl);
    	}
			...
			<div class="subWrap">
				...
				<div class="contents">
						...
						<div class="tableType2" style="border-bottom:0px;">
							<table style="border-bottom:0px;">
								...
								<tbody>
									<tr>
										<th scope="col">작성자</th>
										<td>${awnesForm.userNm}</td>
										<th scope="col">작성일</th>
										<td><fmt:formatDate value="${awnesForm.updDts}" pattern="yyyy.MM.dd HH:mm:ss"/></td>
									</tr>
									<tr>
										<th scope="col">상태</th>
										<td  colspan="3">
											<div class="wrap">
												...
												<form:radiobutton path="dispYn" id="dispN" label="비공개" value="N"/>
											</div>
										</td>
									</tr>
									<tr>
					...
					<div class="section a2 btn">
						...
						<div class="right">
							<a href="#" class="btnType2" id="btn_update">등록</a>

<%--
 * =============================================================================
 * 파일명 : /manage/specialList.jsp
 * 작성자 : 권용재
 * 작성일자 : 2015-12-24
 *
 * 파일 내용 : SPECIAL PRODUCT 리스트
 * =============================================================================
--%>
...
// 	    	 }
	    ...
	    goDetail = function(e){
	    	...
	    	frm.action = "<c:url value='/admin/manage/specialDetail'/>";
	    	frm.submit();
	    };
	    ...
	    $('#btn_reg').click(function() {
	    	...
	    	frm.hiddenEndDtm.value = $("#searchEndDtm").val();
	    	frm.hiddenSearchText.value = $("#searchText").val();
	    	frm.action = "<c:url value='/admin/manage/specialReg'/>";
	    	frm.submit();
	    });
	...
	<div id="wrap">
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<div class="contents">
					<h3><span class="icon1"></span>운영관리 > SPECIAL 관리 > SPECIAL</h3>
										...
										<table>
											...
											<c:choose>
												<c:when test="${not empty specialList}">
													<c:forEach items="${specialList}" var="special" varStatus="listCount">
														<tr>
															<td>${special.rnum}</td>	
															<td>${special.dispYn}</td>
															<td class="tl"><a href="#" onclick="javascript:goDetail(this, '${special.clbltnNm}', '${special.seasonCd}');" data-txtNo="<c:out value='${special.txtNo}'/>">${special.title}</a></td>
															<td>${special.regUserNm}</td>
															<td>${special.updDts}</td>
														</tr>
													</c:forEach>
												</c:when>
												<c:otherwise>

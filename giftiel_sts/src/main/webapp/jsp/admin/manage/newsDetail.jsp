...
// 		ver2.0
    	...
    	if("${newsForm.txtCls}" == "CM00501"){
			$("#clsImg").show();
			$("#clsVideo").hide();
			$("input[id=movUrl]").val("");
		}else if("${newsForm.txtCls}" == "CM00502"){
			$("#clsImg").hide();
			$("#clsVideo").show();
			$("input[id=imgUrl]").val("");
	  ...
	  //파일선택
    	...
    	$("#btn_update").on("click",function(){
		    ...
		    if(!formCheck(frm)) return false;
 	    	...
 	    	$.ajax({
		        url : "<c:url value='/admin/manage/updateNewsProc'/>",
		        type: "post",
    	...
    	$("#btn_del").on("click",function(){
    		...
    		$.ajax({
		        url : "<c:url value='/admin/manage/deleteNewsProc'/>",
		        type: "post",
		        data : $("form#newsForm").serialize(),
		        async: false,
		        success : function(res){
						...
						<div class="tableType2">
							<table>
								...
								<tbody id="contentTbody">
									<tr>
										<th scope="col">작성자</th>
										<td>${newsForm.regUserNm}</td>
										<th scope="col">작성일</th>
										<td>${newsForm.updDts}</td>
									</tr>
									<tr>
										<th scope="col">브랜드</th>
										<td colspan="3">
									...
									<tr>
										...
										<td colspan="3">
											...
											<c:choose>
												<c:when test="${not empty newsForm.thmbUrl }">
													<span class="imgBox"><img id="thmbUrl1" src="<spring:eval expression="@file['upload.image']"/>${newsForm.thmbUrl}" alt="" style="width: 60px; height: 60px;" /></span>
												</c:when>
												<c:otherwise>
									...
									<tr id="mainRow" data-rowNum="main">
										...
										<td colspan="3">
											...
											<div id="clsImg">
												...
												<span class="imgBox">
													<c:choose>
														<c:when test="${not empty newsForm.imgUrl }">
															<img id="imgUrl" src="<spring:eval expression="@file['upload.image']"/>${newsForm.imgUrl}" alt="" style="width: 60px; height: 60px;" />
														</c:when>
													</c:choose>
												</span>
									...
									<c:choose>
										<c:when test="${not empty newsForm.newsCont}">
											<c:forEach var="newsCont" items="${newsForm.newsCont}" varStatus="stat">
											<tr id="addRow${stat.index}" data-rowNum="${stat.index}">
												<th scope="col">슬라이드<br/>이미지/동영상 <br />(<span id="viewArea">${stat.index+2}</span>)<br /><a href="javascript:void(0);" onclick="imgMovieAddRow();" class="btnType2 btnAdd" id="btn_add${stat.index}">추가</a>&nbsp;<a href="javascript:void(0);" onclick="delRow(this);" class="btnType2 btnAdd" id="btn_delRow${stat.index}">삭제</a></th>
												<td colspan="3">
													<input type="hidden"name="newsCont[${stat.index}].addContAddr" id="addContAddr"/>
													<div class="wrap">
														<span>
														<input type="radio" name="newsCont[${stat.index}].addContType" id="txtCls1_${stat.index}" onchange="changeCls(this.value, this);" value="CM00501" <c:if test="${newsCont.addContType=='CM00501'}">checked</c:if>/>
														<label for="txtCls1_${stat.index}">이미지</label>
														</span>
														<span>
														<input type="radio" name="newsCont[${stat.index}].addContType" id="txtCls2_${stat.index}" onchange="changeCls(this.value, this);" value="CM00502" <c:if test="${newsCont.addContType=='CM00502'}">checked</c:if>/>
														<label for="txtCls2_${stat.index}">동영상</label>
														</span>				
													</div>
													<div id="clsImg${stat.index}" <c:if test="${newsCont.addContType=='CM00502'}">style="display: none;"</c:if>>
														<a href="javascript:void(0);" id="btn_file${stat.index}" class="btnType1" onclick="openFilePopup('${stat.index}')">파일선택</a> <span class="addtxt">*이미지 최대 크기는 3MB입니다. 가로 사이즈  700픽셀의 jpg,gif,png 파일로 업로드 바랍니다.</span><br /><br />
														<span class="imgBox"><img id="imgUrl${stat.index}" src="<spring:eval expression="@file['upload.image']"/>${newsCont.addContAddr}" alt="" style="width: 60px; height: 60px;" /></span>
														<input id="imgUrl${stat.index}" name="imgUrl${stat.index}" class="wd_200" readonly="true" type="text" checked="checked" value="${newsCont.addContAddr}"/>
													</div>			
													<div id="clsVideo${stat.index}" <c:if test="${newsCont.addContType=='CM00501'}">style="display: none;"</c:if>>
														<input type="text"  id="movurl${stat.index}" name="movUrl${stat.index}" class="ip_tit" maxlength=500 value="${newsCont.addContAddr}"/>
														<p class="addtxt">* 메인으로 등록할 유투브 동영상 주소를 입력해 주세요.</p>
													</div>
												</td>
											</tr>
											</c:forEach>
...
<!-- 										<td colspan="3"> -->
<!-- 											<div class="wrap"> -->
<%-- 												<form:radiobuttons path="txtCls" id="txtCls" items="${newsClsList}" itemLabel="cdNm" itemValue="cdNo" onchange="chageCls(this.value);"/> --%>
<!-- 											</div> -->
<!-- 										</td> -->
<!-- 									</tr> -->
<!-- 									<tr id="clsImg"> -->
<!-- 										<th scope="col">이미지</th> -->
<!-- 										<td colspan="3"> -->
<!-- 											<a href="javascript:void(0);" id="btn_file" class="btnType1">파일선택</a> <span class="addtxt">*이미지 최대 크기는 3MB입니다. 가로 사이즈  700픽셀의 jpg,gif,png 파일로 업로드 바랍니다.</span><br /><br /> -->
											
<%-- 											<span class="imgBox"><img id="imgUrl" src="<spring:eval expression="@file['upload.image']"/>${newsForm.imgUrl}" alt="" style="width: 60px; height: 60px;" /></span> --%>
<%-- 											<form:input path="imgUrl" id="imgUrl" cssClass="wd_200" readOnly = "true"/><a href="javascript:void(0);" onclick="delImg();"><img src="<c:url value='/resources/admin/images/popup/closeBtn2.gif'/>" alt="" /></a>  --%>
<!-- 										</td> -->
<!-- 									</tr> -->
<!-- 									<tr id="clsVideo"> -->
<!-- 										<th scope="col">동영상</th> -->
<!-- 										<td colspan="3"> -->
<%-- 											<form:input path="movUrl" id="movUrl" cssClass="ip_tit" maxlength="100"/> --%>
<!-- 											<p class="addtxt">* 메인으로 등록할 유투브 동영상 주소를 입력해 주세요.</p> -->
<!-- 										</td> -->
<!-- 									</tr> -->
									<tr>
					...
					<div class="section a2 btn">
						<a href="javascript:void(0);" class="btnType2" id="btn_update">수정</a>
						<a href="javascript:void(0);" class="btnType1" id="btn_cancel" onclick="javascript:window.history.back(-1); return false;">취소</a>
						<a href="javascript:void(0);" class="btnType1" id="btn_del">삭제</a>
					</div>
					<!-- //section -->
				</div>
				</form:form>

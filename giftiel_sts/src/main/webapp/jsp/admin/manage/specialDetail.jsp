<%--
 * =============================================================================
 * 파일명 : /manage/specialDetail.jsp
 * 작성자 : 권용재
 * 작성일자 : 2015-12-24
 *
 * 파일 내용 : SPECIAL PRODUCT 상세
 * =============================================================================
--%>
...
<head>
    ...
    $(function() { 
	    ...
	    $('#btn_update').click(function() {
	    	...
	    	$.ajax({
   		        url : "<c:url value='/admin/manage/specialUpdateAjax'/>",
   		        type: "post",
   		        ...
   		        success : function(res){
   		            if (res > 0) {
   						...
   						frm.action = "<c:url value='/admin/manage/specialList'/>";
			 	        frm.submit();
   					} else {
	    ...
	    $('#btn_delete').on("click",function() {
	    	...
	    	$.ajax({
   		        url : "<c:url value='/admin/manage/specialDeleteAjax'/>",
   		        type: "post",
   		        data : $("form#collabo").serialize(),
   		        async: false,
   		        success : function(res){
	    ...
	    $('#btn_save').click(function() {
	        ...
	        frm.target = '_self';
	        frm.action = '/admin/manage/saveGoodsDisplaySpecial';
	        frm.submit();
	    });
    ...
    function callback_setGoods(goodsData, resultId){
    	...
    	for ( var i = 0; i < goodsData.split("|").length; i++) {
    		ubiGdsNo += goodsData.split("|")[i].split(":")[0];
    		if(i+1 != goodsData.split("|").length){
    			ubiGdsNo += ",";
    		}
		}
    	
    	var formdata = {};
		formdata = "ubiGdsNo=" + ubiGdsNo + "&imgSeq=" + "${collabo.imgSeq}";
    	$.ajax({
	        url : "<c:url value='/admin/manage/specialGoodsAddAjax'/>",
	        type: "post",
	        data : formdata,
	...
	<div id="wrap">
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<!--[s] contents -->
				<div class="contents">
					<h3><span class="icon1"></span>운영관리 > 게시판관리 > SPECIAL PRODUCT</h3>
					<!-- section -->
					...
					<form:form id="collabo" name="collabo" method="post" modelAttribute="collabo" onsubmit="">
						...
						<input type="hidden" id="hiddenEndDtm" name="hiddenEndDtm" value="${hiddenEndDtm}"/>
						<input type="hidden" id="hiddenSearchText" name="hiddenSearchText" value="${hiddenSearchText}"/>		
						<input type="hidden" id="goodsTable" name="goodsTable" />				
...
<%-- 											<span class="imgBox"><img id="test" src="<c:url value='<spring:eval expression="@file['upload.image']"/>${collabo.imgUrl}'/>" alt="" style="width: 60px; height: 60px;" /></span>  --%>
					...
					<div class="section a2">
						<div class="tableTitle">
							<div class="right">
								<a href="#" class="btnType1" data-btnId="btn_goodsLnk">상품추가 </a>
							</div>
						</div>		
								...
								<c:forEach items="${goodsList}" var="goods" varStatus="listCount">
									<tr>
										...
										<td class="p_line5">
											...
											<input type="hidden" id="imgSeq" name="imgSeq" value="${collabo.imgSeq}" style="width:40px;">
											<input type="hidden" id="txtNo" name="txtNo" value="${collabo.txtNo}" style="width:40px;">
										</td>
										</tr>
									</c:forEach>
							</table>

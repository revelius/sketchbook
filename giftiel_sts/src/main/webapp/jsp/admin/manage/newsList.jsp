...
<head>
    ...
    $(function() { 
	    ...
	    goDetail = function(e){
	    	...
	    	frm.action = "<c:url value='/admin/manage/newsDetail'/>";
	    	frm.submit();
	    };
	    ...
	    $('#btn_reg').click(function() {
	    	...
	    	frm.hiddenSearchGb.value = $("#searchGb").val();
	    	frm.hiddenSearchText.value = $("#searchText").val();
	    	frm.action = "<c:url value='/admin/manage/regNews'/>";
	    	frm.submit();
	    });
...
<!-- 					<div class="helpBox"> -->
<!-- 						<ul class="boxLi"> -->
<!-- 							<li>- 브랜드 별 News 목록 입니다.</li> -->
<!-- 							<li>- 브랜드 별 Style / Product / Worksouters / News / Event 등록 가능합니다.</li> -->
									...
									<table>
										...
										<tbody>
											...
											<tr>
												<th>분류</th>
												<td>
													<form:select path="newsType" id="newsType" items="${newsTypeList}" itemLabel="cdNm" itemValue="cdNo"/>
												</td>
											</tr>
											<tr>
												<th>검색</th>
												<td>
													...
													<form:input path="searchText" id="searchText" class="wd_200" />
												</td>
											</tr>
										</tbody>
									</table>
										...
										<table>
											...
											<thead>
												<tr id="sortTr">
													...
													<th scope="row">분류<a id="TXT_TP" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
													<th scope="row">제목</th>
													<th scope="row">작성자</th>
													<th scope="row">작성일<a id="UPD_DTS" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
													<th scope="row">등록일<a id="ETC1" href="#" onclick="javascript:setOrderBy(this);" class="down">▼</a></th>
												</tr>
											</thead>
											<tbody>	
											<c:choose>
												<c:when test="${not empty newsList}">
													<c:forEach items="${newsList}" var="news" varStatus="listCount">
														<tr>
															...
															<td>${news.dispYn}</td>
															<td>${news.txtTp}</td>
															<td class="tl"><a href="#" onclick="javascript:goDetail(this);" data-txtNo="<c:out value='${news.txtNo}'/>">${news.title}</a></td>
															<td>${news.regUserNm}</td>
															<td>${news.updDts}</td>
															<td>${news.etc1}</td>
														</tr>
													</c:forEach>

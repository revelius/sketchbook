...
<head>
    ...
    $(function() { 
    	...
    	$(document).on("click","#btn_update",function(e){ 
	    	...
	    	$.ajax({
   		        url : "<c:url value='/admin/manage/lookbookUpdateAjax'/>",
   		        type: "post",
    	...
    	$(document).on("click","#btn_del",function(e){ 
    		...
    		$.ajax({
   		        url : "<c:url value='/admin/manage/lookbookDelAjax'/>",
   		        type: "post",
   		        data : $("form#lookbook").serialize(),
   		        async: false,
   		        success : function(res){
	    ...
	    $(document).on("click","#btn_addImg",function(e){ 
    		...
    		addImg(imgRowHtml);
   			...
   			$("table[data-sort='sortTable'] tbody").sortable({		
            	...
            	update: function( event, ui ) {
    				for ( var i = 0; i < $(this).find("tr").size(); i++) {
    					$(this).find("input[data-sort='prir']").eq(i).val(i+1);
    					$(this).find("span[data-sort='prir']").eq(i).text(i+1);
    				}
            	}
           	});
   			$( "table[data-sort='sortTable'] tbody" ).disableSelection();
    ...
    goodsLnkHtml += "<td><input type='hidden' name='intgBoardCnts[$count$].boardGds[$subCount$].imgUrl' value='$imgUrl$'/>";
    ...
    <c:forEach items="${lookbook.intgBoardCnts}" var="intgBoardCnts" varStatus="statImg">
    	<c:forEach items="${intgBoardCnts.boardGds}" var="boardGds" varStatus="statGds">
    		arrGdsCnt[imgElementCnt] += 1;
    	</c:forEach>
    	imgElementCnt += 1;
    	arrGdsCnt.push(-1);
    </c:forEach>
    ...
    imgRowHtml += "<th scope='col'>썸네일 이미지</th>";
						...
						<div class="tableType2">
							<table>
								...
								<tbody data-id="mainBody">
									...
									<tr>
										<th scope="col" colspan="2">메인 리스트 썸네일</th>
										<td class="mainImg" colspan="3">
											<input type="hidden" id="imgUrl0" name="intgBoardCnts[0].imgUrl" value="${lookbook.intgBoardCnts[0].imgUrl}"/>
											<input type="hidden" name="intgBoardCnts[0].imgCls" value="CM00601"/>
											<input type="hidden" name="intgBoardCnts[0].prir" value="1"/>
											<span class="imgBox"><img id="imgUrl0" src="<spring:eval expression="@file['upload.image']"/>${lookbook.intgBoardCnts[0].imgUrl}" alt="" style="width: 60px; height: 60px;" /></span> 
											<div class="imgUpload">
									...
									<c:choose>
										<c:when test="${not empty lookbook}">
										<c:set var="cnt" value="0"/>
											<c:forEach items="${lookbook.intgBoardCnts}" var="intgBoardCnts" varStatus="listCount">
												
												<c:if test="${intgBoardCnts.imgCls == 'CM00602'}">
												<c:set var="cnt" value="${cnt+1}"/>
<%-- 												<c:out value="${cnt}"></c:out> --%>
													<tr data-imgRowNum="${cnt}">
														<th rowspan="3"><input type="checkbox" data-chkBox="img" data-imgRowNum="${cnt}" /></th>
														<th scope="col">썸네일 이미지</th>
														<td colspan="3" class="mainImg">
															<input type="hidden" id="imgUrl${cnt}" name="intgBoardCnts[${cnt}].imgUrl" value="${intgBoardCnts.imgUrl}"/>
															<input type="hidden" name="intgBoardCnts[${cnt}].imgCls" value="CM00602" /> 
															<input type="hidden" name="intgBoardCnts[${cnt}].prir" data-imgPrir value="${cnt}" />
															<span class="imgBox"><img id="imgUrl${cnt}" src="<spring:eval expression="@file['upload.image']"/>${intgBoardCnts.imgUrl}" alt="" style="width: 60px; height: 60px;" /></span>
															<div class="imgUpload">
																<a href="#" data-btnId="btn_file" data-target="imgUrl${cnt}" class="btnType1">파일선택</a>
																<p class="addtxt">*이미지 최대 크기는 3MB입니다. 가로 사이즈 700픽셀의 jpg,gif,png 파일로 업로드 바랍니다.</p>
															</div></td>
													</tr>
													<tr data-imgRowNum="${cnt}">
														<th scope="col">디테일 이미지</th>
														<td colspan="3" class="mainImg">
															<input type="hidden" id="dtlImgUrl${cnt}" name="intgBoardCnts[${cnt}].dtlImgUrl" value="${intgBoardCnts.dtlImgUrl}"/>
															<span class="imgBox"><img id="dtlImgUrl${cnt}" src="<spring:eval expression="@file['upload.image']"/>${intgBoardCnts.dtlImgUrl}" alt="" style="width: 60px; height: 60px;" /></span>
															<div class="imgUpload">
																<a href="#" data-btnId="btn_file" data-target="dtlImgUrl${cnt}" class="btnType1">파일선택</a>
																<p class="addtxt">*이미지 최대 크기는 3MB입니다. 가로 사이즈 700픽셀의 jpg,gif,png 파일로 업로드 바랍니다.</p>
															</div></td>
													</tr>
													<tr data-imgRowNum="${cnt}">
...
<!-- 															직접입력 버튼 임시 숨김 -->
<!-- 															<a href="#" class="btnType1" data-btnId="btn_psnlReg">직접입력</a>  -->
															<span class="addtxt">* 연결상품은 최대 50개까지 가능합니다.</span> <br/>
															<div class="tableType1 tableInTbl tblYscroll" style="height: 147px;">
																<table class="" data-sort="sortTable">
																	...
																	<tbody id="linkGds${cnt}" data-count="${cnt}">
																	<c:forEach items="${intgBoardCnts.boardGds}" var="boardGds" varStatus="boardGdsCount">
																		<tr data-count="${cnt}" data-subCount="${boardGdsCount.index}">
																			<td>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].imgUrl" value="${boardGds.imgUrl}"/>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].gdsNm" value="${boardGds.gdsNm}"/>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].ubiGdsNo" value="${boardGds.ubiGdsNo}"/>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].linkUrl" value="${boardGds.linkUrl}"/>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].slPrc" value="${boardGds.slPrc}"/>
																				<input type="hidden" name="intgBoardCnts[${cnt}].boardGds[${boardGdsCount.index}].prir" value="${boardGds.prir}" data-sort="prir"/>
																				<input type="checkbox" data-chkBox="gds" data-gdsRowNum="${boardGdsCount.index}"/>
																			</td>
																			<td>${boardGds.gubun}</td>
																			<td class="tl">${boardGds.gdsNm}</td>
																			<td>${boardGds.linkUrl}</td>
																			<td class="tr">${boardGds.slPrc}</td>
																			<td><span data-sort="prir">${boardGds.prir}</span></td>
																		</tr>
																	</c:forEach>
																	</tbody>
																</table>
															...
															<div class="btn">
																<div class="right">
																	<a href="#" class="btnType2" id="btn_gesDel">삭제</a>
																</div>
															</div>
														</td>
													</tr>
					...
					<div class="section a2 btn">
						...
						<div class="right">
							...
							<a href="#" class="btnType2" id="btn_del">삭제</a>
						</div>
					</div>
					<!-- //section -->
				</div>

<%--
 * =============================================================================
 * 파일명 : /manage/obeyAwarenessList.jsp
 * 작성자 : 김진성
 * 작성일자 : 2015-08-06
 *
 * 파일 내용 : Obey 특화 메뉴 Awareness
 * =============================================================================
--%>
...
<head>
	...
	<script type="text/javascript">
	 $(function() {
		    ...
		    goSubmit = function(){
		    	 var frm = document.schForm;
			        frm.action = "<c:url value='/admin/manage/obeyAwarenessList'/>";
			        frm.submit();
		    };
		  ...
		  //상세페이지 이동
		    goDetail = function(e){
		    	var $target = $(e);
		    	var awnesNo = $target.attr('data-awnesNo');
		    	var frm = document.hiddenForm;
		    	frm.awnesNo.value = awnesNo;
		    	frm.action = "<c:url value='/admin/manage/obeyAwarenessDetail'/>";
		    	frm.submit();
		    };
		    ...
		    $('#btn_reg').click(function() {
		    	var frm = document.hiddenForm;
		    	frm.action = "<c:url value='/admin/manage/obeyAwarenessReg'/>";
		    	frm.submit();
		    });
		...
		<div id="container">			
			...
			<div class="subWrap">
				...
				<form id="hiddenForm" name="hiddenForm" method="post">
					<input type="hidden" id="awnesNo" name="awnesNo" value=""/>
				</form>
...
<!-- 					<div class="helpBox"> -->
<!-- 						<ul class="boxLi"> -->
<!-- 							<li>- Awareness 목록 입니다.</li> -->
<!-- 							<li>- Awareness는 1개의 게시물만 공개 가능합니다.</li> -->
<!-- 							<li>- 등록된 Awareness는 OBEY > Awareness에서 확인 가능합니다.</li> -->
<!-- 						</ul> -->
								...
								<div class="section a2">
									...
									<div class="tableType1">
										<table>
											...
											<tbody>	
											<c:choose>
												<c:when test="${not empty awnesList}">
													<c:forEach items="${awnesList}" var="awnes" varStatus="listCount">
														<tr>
															<td>${awnes.rnum}</td>	
															<td>${awnes.dispYn}</td>
															<td class="tl"><a href="#" onclick="javascript:goDetail(this);" data-awnesNo="<c:out value='${awnes.awnesNo}'/>">${awnes.title}</a></td>
															<td>${awnes.userNm}</td>
															<td><fmt:formatDate value="${awnes.updDts}" pattern="yyyy.MM.dd"/></td>
														</tr>
													</c:forEach>
												</c:when>
												<c:otherwise>

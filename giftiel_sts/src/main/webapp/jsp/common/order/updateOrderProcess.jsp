<%--
 * =============================================================================
 * 파일명 : /common/order/updateOrderProcess.jsp
 * 작성자 : 권용재
 * 작성일자 : 2015-06-29
 *
...
<%
    ...
    /* ============================================================================== */
    String vcnt_yn           = f_get_parm( request.getParameter( "vcnt_yn"          ) ); // 상태변경시 계좌이체, 가상계좌 여부
    /* = -------------------------------------------------------------------------- = */
    ...
    if("STE1".equals(mod_type)){ //배송시작
    	deli_numb = request.getParameter("deli_numb");
    	deli_corp = request.getParameter("deli_corp");
    }else if("STE2".equals(mod_type)){ //즉시취소
    	
    }else if("STE3".equals(mod_type)){ //정산보류
    	
    }else if("STE4".equals(mod_type)){ //취소
    	
    }else if("STE5".equals(mod_type)){ //발급계좌해지(가상계좌)
        y_mod_desc_cd = "CA06";
        y_mod_desc = "주문취소";
    }else if("STE9_C".equals(mod_type)){ //취소(카드)
    	
    }else if("STE9_CP".equals(mod_type)){ //부분취소(카드)
    	
    }else if("STE9_A".equals(mod_type)){ // 취소(실시간계좌이체)
    	
    }else if("STE9_AP".equals(mod_type)){ // 부분취소(실시간계좌이체)
    	
    }else if("STE9_AR".equals(mod_type)){ // 환불(실시간계좌이체)
    	
    }else if("STE9_V".equals(mod_type)){ // 환불(가상계좌)
    	
    }else if("STE9_VP".equals(mod_type)){ // 부분환불(가상계좌)
    ...
    if ( req_tx.equals( "mod_escrow" ) )
    {
        ...
        if( mod_type.equals("STE9_C")  || mod_type.equals("STE9_CP") ||
            mod_type.equals("STE9_A")  || mod_type.equals("STE9_AP") ||
            mod_type.equals("STE9_AR") || mod_type.equals("STE9_V")  ||
            mod_type.equals("STE9_VP") )
        {
            ...
            else if( mod_type.equals("STE9_VP") )
            {
                ...
                c_PayPlus.mf_set_us( mod_data_set_no, "mod_mny"         , y_mod_mny         );
                c_PayPlus.mf_set_us( mod_data_set_no, "mod_sub_type"    , "MDSC04"          );
                c_PayPlus.mf_set_us( mod_data_set_no, "mod_bankcode"    , "BK"+y_bank_code       );
                c_PayPlus.mf_set_us( mod_data_set_no, "mod_account"     , y_refund_account  );
                c_PayPlus.mf_set_us( mod_data_set_no, "mod_depositor"   , y_refund_nm       );
                //c_PayPlus.mf_set_us( mod_data_set_no, "tax_flag"        , "TG03"            ); // 복합과세 부분취소
        ...
        else
        {
            ...
            {
                c_PayPlus.mf_set_us( mod_data_set_no, "deli_numb", deli_numb );      // 운송장 번호
                c_PayPlus.mf_set_us( mod_data_set_no, "deli_corp", deli_corp );      // 택배 업체명
            }
            if ( mod_type.equals("STE2") || mod_type.equals("STE4") )                                       // 상태변경 타입이 [즉시취소] 또는 [취소]인 계좌이체, 가상계좌의 경우
            {
                if ( vcnt_yn.equals("Y") )
                {
                    ...
                    c_PayPlus.mf_set_us( mod_data_set_no, "bank_code",   mod_bankcode      );  // 환불수취은행코드
    ...
    /* = -------------------------------------------------------------------------- = */
        	...
        	if ( res_cd.equals( "0000" ) ){
                ...
                if("STE1".equals(mod_type)){ //배송시작
                	orderResult.setEscrowDlvYn("Y");
                	orderResult.setPerOrdStat(perOrdStat); //배송중
                ...
                if("STE2".equals(mod_type) || "STE4".equals(mod_type) || "STE5".equals(mod_type) || "STE9_A".equals(mod_type) || "STE9_AP".equals(mod_type) || "STE9_AR".equals(mod_type) || "STE9_V".equals(mod_type) || "STE9_VP".equals(mod_type)){
                	//사용한 적립금 환불
...
//             		//가상계좌사용중지가 아니면서 회원일때만 적립된 적립금 회수로직 실행
...
//             		}
        		...
        		if("OR00507".equals(perOrdStat)){ //주문취소
        			...
        			smsSend.sendSMS(6, order.getOrdrCp(), order.getSiteNm(), order.getRcpnNm(), order.getOrdNo(),0);
        			//MAIL 발송
                ...
                if("OR00504".equals(perOrdStat)){ //배송시작
                	//SMS발송(SMS문구번호, 수신번호, 브랜드명, 이름, 주문번호) - null시 ""
        			smsSend.sendSMS(5, order.getOrdrCp(), order.getSiteNm(), order.getRcpnNm(), order.getOrdNo(), ordGdsSeq);
        			result = mailSend.sendMail(order.getOrdrNm(),order.getOrdrEmail(),request,hm,"shippComp");
                }else if("OR00509".equals(perOrdStat)){ //교환진행
        			smsSend.sendSMS(8, order.getOrdrCp(), order.getSiteNm(), order.getRcpnNm(), order.getOrdNo(), ordGdsSeq);
					hm.put("ordGdsSeq", order.getOrderGoods().get(orderGdsSeq).getOrdGdsSeq());
        			result = mailSend.sendMail(order.getOrdrNm(),order.getOrdrEmail(),request,hm,"ordReturn");
                }else if("OR00513".equals(perOrdStat)){ //반품진행
        			smsSend.sendSMS(9, order.getOrdrCp(), order.getSiteNm(), order.getRcpnNm(), order.getOrdNo(), ordGdsSeq);
					hm.put("ordGdsSeq", order.getOrderGoods().get(orderGdsSeq).getOrdGdsSeq());
        			result = mailSend.sendMail(order.getOrdrNm(),order.getOrdrEmail(),request,hm,"ordChange");
                }
            } 
        }
    ///* ============================================================================== */
...
<html xmlns="http://www.w3.org/1999/xhtml" >
    ...
    <input type="text" id="result_code" name="result_code" value="<%=res_cd%>">
    <br/>
	<br/><%=res_msg%>
    </body>
</html>

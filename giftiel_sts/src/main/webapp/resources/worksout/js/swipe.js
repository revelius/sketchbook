...
//    }
    ...
    element.style.width = (slides.length * width+1) + 'px';
  ...
  function setcont(){
	...
	$("span[class='util01']").children().text((num)+"/"+total);
	
	var contCls = $("div[class='awareness_item']").eq(index).find("#contCls").val();

	if(contCls == "CM00501"){//이미지
		$("a[class='btn_download']").attr("data-file",$("div[class='awareness_item']").eq(index).find("img").attr("data-imgUrl"));
		$("[data-view]").hide();
		...
		$("[data-view='CM00503']").attr("href",$("div[class='awareness_item']").eq(index).find("img").attr("data-contUrl"));
		$("[data-view]").hide();
